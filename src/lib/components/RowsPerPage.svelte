...
<Listbox value={$options.rowsPerPage} on:change={(e) => {
	...
	<ListboxButton class="bg-white dark:bg-gray-300 relative h-8 w-full pl-2 border-r rounded-tl-md rounded-bl-md text-left sm:text-sm dark:text-gray-600">
		{$options.rowsPerPage}
	...
	<ListboxOptions class="absolute z-10 mt-1 w-24 bg-white dark:bg-gray-300 shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
